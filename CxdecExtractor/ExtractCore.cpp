...
namespace Engine
{
	...
	void ExtractCore::ExtractPackage(const std::wstring& packageFileName)
	{
		...
		if (!entries.empty())
		{
			FullCreateDirectoryW(this->mExtractDirectoryPath);     //创建资源提取导出文件夹

			std::wstring extractOutput = this->mExtractDirectoryPath + L"\\";	//导出文件夹
			...
			{
				std::wstring dirHash = BytesToHexStringW(entry.DirectoryPathHash, sizeof(entry.DirectoryPathHash));  //文件夹Hash字符串形式
				std::wstring fileNameHash = BytesToHexStringW(entry.FileNameHash, sizeof(entry.FileNameHash));     //文件名Hash字符串形式
			...
			MessageBoxW(nullptr, (packageFileName + L"提取成功").c_str(), L"提示", MB_OK);
			fileTable.Close();
		}
		else
		{
